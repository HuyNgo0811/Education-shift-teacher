document.addEventListener("DOMContentLoaded",function () { 
  ...
  const selectTeacher = document.querySelector('#idinput');
  selectTeacher.addEventListener("change",function() {
    showclass();
    setTimeout(showclass, 100);
  })

  function showclass() {
    fetch('/tkb/teacher-json')
    .then(response => response.json())
    .then(teacherdata => {
      teacherdata.forEach(t => {
        const idinput = document.getElementById('idinput').value; //teacherID
        for(let i=1;i<157;i++){
          let buoi = 'buoi'+i;
          let clss = 'class'+i;
          let classout = document.querySelector('#'+clss);
          if(t.teacherID === idinput){
            const input = document.createElement('input');
            input.disabled=true;
            input.value=t[buoi];
            classout.innerHTML='';
            classout.appendChild(input);
            let row = document.querySelector('#row'+i);
            row.style.display='';
          }
        }
